                  ...
                  {issue.timelineItems.edges.length > 0 && (
                    <React.Fragment>
                      {issue.timelineItems.edges
                        .filter(
                          item => item.node.source.__typename === "PullRequest"
                        )
                        .map(node => (
                          <Grid item key={node.node.source.id}>
                            <PrIcon pr={node.node.source} />
                          </Grid>
                        ))}
                      {issue.timelineItems.edges
                        .filter(
                          item => item.node.target.__typename === "PullRequest"
                        )
                        .map(node => (
                          <Grid item key={node.node.target.id}>
                            <PrIcon pr={node.node.target} />
                          </Grid>
                        ))}
                    </React.Fragment>
                  )}
