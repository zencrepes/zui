...
export const githubWatchers: GithubWatchers = {
  ...
  effects: (dispatch: Dispatch) => ({
    async initView() {
      ...
      logger.info('githubWatchers Logger initialized');
      dispatch.githubWatchers.setLog(logger);
    },
    ...
    async updateQueryIfDifferent(newQuery: any, rootState: any) {
      const originalQuery = rootState.githubWatchers.query;
      ...
      if (JSON.stringify(originalQuery) !== JSON.stringify(newQuery)) {
        if (newQuery === null) {
          dispatch.githubWatchers.setQuery({});
        } else {
          dispatch.githubWatchers.setQuery(newQuery);
        }
      }
    },

    async updateTabIfDifferent(newTab: any, rootState: any) {
      const originalTab = rootState.githubWatchers.selectedTab;
      if (originalTab !== newTab) {
        dispatch.githubWatchers.setSelectedTab(newTab);
      }
    },
  }),
};
