...
export const testingRuns: TestingStates = {
  state: {
    log: {},
    loading: false,
    selectedTab: 'explore',
  ...
  effects: (dispatch: Dispatch) => ({
    async initView() {
      ...
      logger.info('testingRuns Logger initialized');
      dispatch.testingRuns.setLog(logger);
    },
    ...
    async updateQueryIfDifferent(newQuery: any, rootState: any) {
      const originalQuery = rootState.testingRuns.query;
      ...
      if (JSON.stringify(originalQuery) !== JSON.stringify(newQuery)) {
        if (newQuery === null) {
          dispatch.testingRuns.setQuery({});
        } else {
          dispatch.testingRuns.setQuery(newQuery);
        }
      }
    },

    async updateTabIfDifferent(newTab: any, rootState: any) {
      const originalTab = rootState.testingRuns.selectedTab;
      if (originalTab !== newTab) {
        dispatch.testingRuns.setSelectedTab(newTab);
      }
    },
  }),
};
