...
export const testingStates: TestingStates = {
  state: {
    log: {},
    loading: false,
    selectedTab: 'explore',
  ...
  effects: (dispatch: Dispatch) => ({
    async initView() {
      ...
      logger.info('testingStates Logger initialized');
      dispatch.testingStates.setLog(logger);
    },
    ...
    async updateQueryIfDifferent(newQuery: any, rootState: any) {
      const originalQuery = rootState.testingStates.query;
      ...
      if (JSON.stringify(originalQuery) !== JSON.stringify(newQuery)) {
        if (newQuery === null) {
          dispatch.testingStates.setQuery({});
        } else {
          dispatch.testingStates.setQuery(newQuery);
        }
      }
    },

    async updateTabIfDifferent(newTab: any, rootState: any) {
      const originalTab = rootState.testingStates.selectedTab;
      if (originalTab !== newTab) {
        dispatch.testingStates.setSelectedTab(newTab);
      }
    },
  }),
};
