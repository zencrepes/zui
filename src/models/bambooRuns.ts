...
interface BambooRuns {
  state: any;
  reducers: any;
  effects: any;
}

export const bambooRuns: BambooRuns = {
  state: {
    log: {},
    loading: false,
    selectedTab: 'explore',
  ...
  effects: (dispatch: Dispatch) => ({
    async initView() {
      ...
      logger.info('bambooRuns Logger initialized');
      dispatch.bambooRuns.setLog(logger);
    },
    ...
    async updateQueryIfDifferent(newQuery: any, rootState: any) {
      const originalQuery = rootState.bambooRuns.query;
      ...
      if (JSON.stringify(originalQuery) !== JSON.stringify(newQuery)) {
        if (newQuery === null) {
          dispatch.bambooRuns.setQuery({});
        } else {
          dispatch.bambooRuns.setQuery(newQuery);
        }
      }
    },

    async updateTabIfDifferent(newTab: any, rootState: any) {
      const originalTab = rootState.bambooRuns.selectedTab;
      if (originalTab !== newTab) {
        dispatch.bambooRuns.setSelectedTab(newTab);
      }
    },
  }),
};
