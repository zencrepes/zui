...
interface CircleciInsights {
  state: any;
  reducers: any;
  effects: any;
}

export const circleciInsights: CircleciInsights = {
  state: {
    log: {},
    loading: false,
    selectedTab: 'explore',
  ...
  effects: (dispatch: Dispatch) => ({
    async initView() {
      ...
      logger.info('circleciInsights Logger initialized');
      dispatch.circleciInsights.setLog(logger);
    },
    ...
    async updateQueryIfDifferent(newQuery: any, rootState: any) {
      const originalQuery = rootState.circleciInsights.query;
      ...
      if (JSON.stringify(originalQuery) !== JSON.stringify(newQuery)) {
        if (newQuery === null) {
          dispatch.circleciInsights.setQuery({});
        } else {
          dispatch.circleciInsights.setQuery(newQuery);
        }
      }
    },

    async updateTabIfDifferent(newTab: any, rootState: any) {
      const originalTab = rootState.circleciInsights.selectedTab;
      if (originalTab !== newTab) {
        dispatch.circleciInsights.setSelectedTab(newTab);
      }
    },
  }),
};
