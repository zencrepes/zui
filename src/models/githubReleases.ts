...
export const githubReleases = {
  ...
  effects: (dispatch: Dispatch) => ({
    async initView() {
      ...
      logger.info('githubReleases Logger initialized');
      dispatch.githubReleases.setLog(logger);
    },
    ...
    async updateQueryIfDifferent(newQuery: any, rootState: any) {
      const originalQuery = rootState.githubReleases.query;
      ...
      if (JSON.stringify(originalQuery) !== JSON.stringify(newQuery)) {
        if (newQuery === null) {
          dispatch.githubReleases.setQuery({});
        } else {
          dispatch.githubReleases.setQuery(newQuery);
        }
      }
    },

    async updateTabIfDifferent(newTab: any, rootState: any) {
      const originalTab = rootState.githubReleases.selectedTab;
      if (originalTab !== newTab) {
        dispatch.githubReleases.setSelectedTab(newTab);
      }
    },
  }),
};
