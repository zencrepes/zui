...
export const circleciPipelines = {
  ...
  effects: (dispatch: Dispatch) => ({
    async initView() {
      ...
      logger.info('circleciPipelines Logger initialized');
      dispatch.circleciPipelines.setLog(logger);
    },
    ...
    async updateQueryIfDifferent(newQuery: any, rootState: any) {
      const originalQuery = rootState.circleciPipelines.query;
      ...
      if (JSON.stringify(originalQuery) !== JSON.stringify(newQuery)) {
        if (newQuery === null) {
          dispatch.circleciPipelines.setQuery({});
        } else {
          dispatch.circleciPipelines.setQuery(newQuery);
        }
      }
    },

    async updateTabIfDifferent(newTab: any, rootState: any) {
      const originalTab = rootState.circleciPipelines.selectedTab;
      if (originalTab !== newTab) {
        dispatch.circleciPipelines.setSelectedTab(newTab);
      }
    },
  }),
};
