...
export const githubVulnerabilities: GithubVulnerabilities = {
  ...
  effects: (dispatch: Dispatch) => ({
    async initView() {
      ...
      logger.info('githubVulnerabilities Logger initialized');
      dispatch.githubVulnerabilities.setLog(logger);
    },
    ...
    async updateQueryIfDifferent(newQuery: any, rootState: any) {
      const originalQuery = rootState.githubVulnerabilities.query;
      ...
      if (JSON.stringify(originalQuery) !== JSON.stringify(newQuery)) {
        if (newQuery === null) {
          dispatch.githubVulnerabilities.setQuery({});
        } else {
          dispatch.githubVulnerabilities.setQuery(newQuery);
        }
      }
    },

    async updateTabIfDifferent(newTab: any, rootState: any) {
      const originalTab = rootState.githubVulnerabilities.selectedTab;
      if (originalTab !== newTab) {
        dispatch.githubVulnerabilities.setSelectedTab(newTab);
      }
    },
  }),
};
