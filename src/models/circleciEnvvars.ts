...
export const circleciEnvvars: CircleciEnvvars = {
  ...
  effects: (dispatch: Dispatch) => ({
    async initView() {
      ...
      logger.info('circleciEnvvars Logger initialized');
      dispatch.circleciEnvvars.setLog(logger);
    },
    ...
    async updateQueryIfDifferent(newQuery: any, rootState: any) {
      const originalQuery = rootState.circleciEnvvars.query;
      ...
      if (JSON.stringify(originalQuery) !== JSON.stringify(newQuery)) {
        if (newQuery === null) {
          dispatch.circleciEnvvars.setQuery({});
        } else {
          dispatch.circleciEnvvars.setQuery(newQuery);
        }
      }
    },

    async updateTabIfDifferent(newTab: any, rootState: any) {
      const originalTab = rootState.circleciEnvvars.selectedTab;
      if (originalTab !== newTab) {
        dispatch.circleciEnvvars.setSelectedTab(newTab);
      }
    },
  }),
};
