...
const CountsBuckets: React.FC<Props> = (props: Props) => {
  ...
  if (data !== undefined) {
    const chartData = {
      datasets: [
        {
          milestone: 'Number of Milestones',
          backgroundColor: '#64b5f6',
          borderColor: '#64b5f6',
          borderWidth: 2,
          data: prepBuckets
            .filter((b: any) => data.githubMilestones[b.key] !== undefined)
            .map((b: any) => data.githubMilestones[b.key].items.totalCount),
        },
      ],
      milestones: buckets.filter((b: any) => data.githubMilestones[b.key] !== undefined).map((b: any) => b.name),
    };
    return (
