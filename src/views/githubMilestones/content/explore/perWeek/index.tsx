...
const PerWeek: React.FC<Props> = (props: Props) => {
  ...
  if (data !== undefined) {
    ...
    for (const item of data.githubMilestones.data.createdAt.buckets) {
    ...
    for (const x of datapoints) {
      const createdAt = data.githubMilestones.data.createdAt.buckets.find((item: any) => item.keyAsString === x);
      dataseries.push({
        milestone: x,
    ...
    const chartData = {
      datasets: [
        ...
        {
          label: 'New Milestones (rolling average)',
      ...
      labels: dataseries.map((item: any) => format(parseISO(item.milestone), 'LLL do yyyy')),
    };
