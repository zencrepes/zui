...
const QuickNumbers: React.FC<Props> = (props: Props) => {
  ...
  const cards = [
    {
      key: 1,
      count: data.githubMilestones.currentQuery.items.totalCount,
      query: query,
      title: 'In current query',
    },
    {
      key: 2,
      count: data.githubMilestones.thirtyDays.items.totalCount,
      query: buildQuery(query, thirtyDays),
      title: 'Created in the last 30 days',
    },
    // {
    //   key: 3,
    //   count: data.githubMilestones.inactive.items.totalCount,
    ...
    //   count: data.githubMilestones.inactive.items.totalCount,
    //   query: buildQuery(query, inactive),
    //   title: 'Colors used across milestones (TO BE ADDED)',
    // },
  ];
