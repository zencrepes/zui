...
const PerWeek: React.FC<Props> = (props: Props) => {
  ...
  if (data !== undefined) {
    ...
    for (const item of data.testingStates.data.createdAt.buckets) {
    ...
    for (const x of datapoints) {
      const createdAt = data.testingStates.data.createdAt.buckets.find((item: any) => item.keyAsString === x);
      dataseries.push({
        state: x,
    ...
    const chartData = {
      datasets: [
        ...
        {
          label: 'Updated States (rolling average)',
      ...
      labels: dataseries.map((item: any) => format(parseISO(item.state), 'LLL do yyyy')),
    };
