...
const PerWeek: React.FC<Props> = (props: Props) => {
  ...
  if (data !== undefined) {
    ...
    for (const item of data.githubReleases.data.createdAt.buckets) {
    ...
    for (const x of datapoints) {
      const createdAt = data.githubReleases.data.createdAt.buckets.find((item: any) => item.keyAsString === x);
      dataseries.push({
        release: x,
    ...
    const chartData = {
      datasets: [
        ...
        {
          label: 'New Releases (rolling average)',
      ...
      labels: dataseries.map((item: any) => format(parseISO(item.release), 'LLL do yyyy')),
    };
