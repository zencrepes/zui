...
const ClosedPerWeek: React.FC<Props> = (props: Props) => {
  ...
  if (data !== undefined) {
    ...
    for (const item of data.circleciPipelines.data.closedAt.buckets) {
    ...
    for (const x of datapoints) {
      const closedAt = data.circleciPipelines.data.closedAt.buckets.find((item: any) => item.keyAsString === x);
      dataseries.push({
        label: x,
        closedAt: closedAt !== undefined ? closedAt.docCount : 0,
        closedAtAvg: closedAt !== undefined ? closedAt.movinb : 0,
      });
    }
    ...
    const chartData = {
      datasets: [
        ...
        {
          label: 'Triggered Pipelines (rolling average)',
