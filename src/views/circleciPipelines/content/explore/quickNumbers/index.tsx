...
const QuickNumbers: React.FC<Props> = (props: Props) => {
  ...
  const sevenDaysCreated = [{ op: '>=', content: { field: 'createdAt', value: sevenDaysPrior } }];
  const sevenDaysClosed = [{ op: '>=', content: { field: 'closedAt', value: sevenDaysPrior } }];
  ...
  const cards = [
    {
      key: 1,
      count: data.circleciPipelines.currentQuery.count,
      query: query,
      title: 'In current query',
    },
    {
      key: 2,
      count: data.circleciPipelines.sevenDaysCreated.count,
      query: buildQuery(query, sevenDaysCreated),
      title: 'Created in the past 7 days',
    },
    {
      key: 3,
      count: data.circleciPipelines.sevenDaysClosed.count,
      query: buildQuery(query, sevenDaysClosed),
      title: 'Closed in the past 7 days',
    },
  ];
