...
const PerWeek: React.FC<Props> = (props: Props) => {
  ...
  const { data } = useQuery(GQL_QUERY, {
    variables: {
      ...
      aggOptionsAllCases: JSON.stringify({ calendarInterval: 'week', sumField: 'caseSuccess' }),
      aggOptionsFailedCases: JSON.stringify({ calendarInterval: 'week', sumField: 'caseFailure' }),
    },
  ...
  if (data !== undefined) {
    ...
    for (const item of data.testingCases.data.allCases.buckets) {
    ...
    for (const x of datapoints) {
      const createdAtSuccess = data.testingCases.data.allCases.buckets.find((item: any) => item.keyAsString === x);
      dataseriesSuccess.push({
        state: x,
        createdAt: createdAtSuccess !== undefined ? createdAtSuccess.sum : 0,
      });

      const createdAtFailed = data.testingCases.data.failedCases.buckets.find((item: any) => item.keyAsString === x);
